...
Introduction
------------
Corpora, which are crawled or downloaded from the Internet, have diverse
...
*Twitter* consists of tweets. Furthermore, **structures are even changed by
platforms of corpus contents**. The same wiki contents can have different
...
``__extension__`` variable must be in global scope of the module. All
...
should be defined in ``__extension__`` variable. The implementation function
...
it to plain text. After splitting the text into single sentences, save the
...
.. note::
    ...
    recommends using :ref:`utils for extension` when creating
